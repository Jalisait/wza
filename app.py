...
def preprocess_for_prediction(input_path: str, file_type: str):
    print("📥 Chargement du fichier à prédire...")

    if file_type == "csv":
        X_new = pd.read_csv(input_path)
    elif file_type == "json":
        X_new = pd.read_json(input_path)
    else:
        raise ValueError("Format de fichier non supporté. Veuillez fournir un fichier CSV ou JSON.")
...
async def predict_freq(file: UploadFile = File(...)):
    ...
    file_type = "csv" if suffix == ".csv" else "json" if suffix == ".json" else None

    if not file_type:
        return {"error": "Seuls les fichiers .csv ou .json sont acceptés."}
    ...
    X_to_predict = preprocess_for_prediction(tmp_path, file_type)
...
async def predict_montant(file: UploadFile = File(...)):
    ...
    result = pd.DataFrame({"ID": X_to_predict["ID"], "CM": y_pred_cm})
    return result.to_dict(orient="records")
